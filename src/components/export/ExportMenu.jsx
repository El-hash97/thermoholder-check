...
export default function ExportMenu({ data }) {
  ...
  const lastEntry = data?.entries?.slice().sort((a, b) => b.timestamp?.localeCompare(a.timestamp ?? ''))[0] ?? null
    ...
    <div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-xl text-sm font-medium text-white touch-target transition-colors bg-blue-700 hover:bg-blue-600 ${loading ? 'opacity-60' : ''}`}
        >
          <Share2 size={16} />
          {loading ? '...' : 'Share'}
        </button>
